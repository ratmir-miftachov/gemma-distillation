from transformers import Gemma4Config


class MonarchGemma4Config(Gemma4Config):
    """Gemma 4 configuration with selected MLPs replaced by Monarch factors."""

    model_type = "monarch_gemma4"

    def __init__(
        self,
        monarch_compressed_layers=None,
        monarch_blocks_weights=128,
        monarch_factor_count=2,
        monarch_format_version=1,
        monarch_base_model="google/gemma-4-E2B-it",
        monarch_lora_rank=0,
        monarch_lora_alpha=1.0,
        monarch_lora_dropout=0.0,
        monarch_lora_target_projections=None,
        monarch_lora_format_version=1,
        **kwargs,
    ):
        super().__init__(**kwargs)
        self.monarch_compressed_layers = list(monarch_compressed_layers or [])
        self.monarch_blocks_weights = int(monarch_blocks_weights)
        self.monarch_factor_count = int(monarch_factor_count)
        self.monarch_format_version = int(monarch_format_version)
        self.monarch_base_model = str(monarch_base_model)
        self.monarch_lora_rank = int(monarch_lora_rank)
        self.monarch_lora_alpha = float(monarch_lora_alpha)
        self.monarch_lora_dropout = float(monarch_lora_dropout)
        self.monarch_lora_target_projections = list(
            monarch_lora_target_projections or ["gate_proj", "up_proj", "down_proj"]
        )
        self.monarch_lora_format_version = int(monarch_lora_format_version)

        if self.monarch_factor_count != 2:
            raise ValueError("this model format supports exactly two Monarch factors")
        if len(set(self.monarch_compressed_layers)) != len(self.monarch_compressed_layers):
            raise ValueError("monarch_compressed_layers contains duplicate layer indices")
        if self.monarch_lora_rank < 0:
            raise ValueError("monarch_lora_rank must be nonnegative")
        if not 0.0 <= self.monarch_lora_dropout < 1.0:
            raise ValueError("monarch_lora_dropout must be in [0, 1)")
        expected_targets = {"gate_proj", "up_proj", "down_proj"}
        if set(self.monarch_lora_target_projections) != expected_targets:
            raise ValueError(
                "this model format requires LoRA targets gate_proj, up_proj, and down_proj"
            )
